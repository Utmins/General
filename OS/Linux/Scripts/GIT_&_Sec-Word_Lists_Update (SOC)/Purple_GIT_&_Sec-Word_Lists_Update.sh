#!/bin/bash
# Purple_GIT_&_Sec-Word_Lists_Update_v8.sh (no-CSV build)
# All-in-one maintainer for cybersec wordlists & repos.
# - Interactive menu with aligned layout + English explanations
# - Update ALL git repos (system-wide), pulling as repo owner
# - Fetch popular cyber wordlists (GitHub)
# - Optional: include Assetnote wordlists repo
# - Optional: APT bootstrap (+ prime symlinks/wordlists like rockyou.txt)
# - Auto-decompress .tar.gz/.tgz + single .gz under /usr/share/wordlists
# - Universal "matryoshka" autoclean after extraction (default ON)
# - Optional: skip SecLists Zip-Bombs from extraction
# - Optional: Symlink ALL discovered wordlists into one folder
# - Optional: Deduplicate APT vs GIT duplicates with APT preference (+ report)
# - Clean log file (no ANSI), colorful terminal output
#
# Log: ~/Git_Update_Reports/run_*.txt
#
# v8 changes:
#   * Added toggle b) APT bootstrap — install core tools via apt and prime wordlists (e.g., rockyou)
#   * Added toggle n) Include assetnote — add assetnote/wordlists to cloned repos
#   * Added CLI flags --apt-bootstrap/--no-apt-bootstrap and --include-assetnote/--no-include-assetnote
#   * Kept defaults exactly as before (Zip-Bombs OFF by default, etc.)
# no-CSV build:
#   * Removed generation of wordlists_all_links_list.csv and _all-links_list.txt
#   * Hardened CLI args loop to ignore empty args
...
# ===== Logging (enable AFTER menu; log stays clean via sed) =====
REPORT_DIR="$HOME/Git_Update_Reports"
TS="$(date +%Y-%m-%d_%H-%M-%S)"
RUN_LOG="$REPORT_DIR/run_${TS}.txt"
mkdir -p "$REPORT_DIR"

# ===== Defaults (toggle in menu) =====
SHOW_MENU=true
DRY_RUN=false
SCAN_SYSTEM=true
CROSS_FS=false
...
APT_BOOTSTRAP=false
INCLUDE_ASSETNOTE=false

# ===== CLI flags (menu still default) =====
for arg in "$@"; do
  [[ -z "$arg" ]] && continue
  case "$arg" in
...
# Declare base list (without Assetnote; it can be toggled in later)
declare -A WORDLIST_REPOS=(
  ...
  # ["assetnote-wordlists"]="https://github.com/assetnote/wordlists.git"   # gated by INCLUDE_ASSETNOTE
)

# ===== Helpers =====
onoff(){ [[ "$1" == true ]] && printf "ON " || printf "OFF"; }

printf_cfg_line(){ # status label explanation
  printf "  [%-3s] %-26s — %s\n" "$(onoff "$1")" "$2" "$3"
}
...
# --- Universal matryoshka helpers ---
is_zipbomb_path() {
  case "$1" in
    *"/SecLists/Payloads/Zip-Bombs/"*) return 0;;
    *) return 1;;
  esac
}
...
tar_topdir() {
  local tarfile="$1"
  local top
  top="$(tar -tzf "$tarfile" 2>/dev/null | head -1 | cut -d/ -f1)"
  [[ -n "$top" ]] && echo "$top" || echo ""
}
...
is_recursive_tar() {
  local tarfile="$1"
  local listing
  listing="$(tar -tzf "$tarfile" 2>/dev/null)"
  [[ -n "$listing" ]] || return 1
  local nested_count
  nested_count="$(echo "$listing" | grep -E '\.(tar\.gz|tgz)$' | wc -l | tr -d ' ')"
  local files_count
  files_count="$(echo "$listing" | grep -v '/$' | wc -l | tr -d ' ')"
  if [[ "$nested_count" == "1" && "$files_count" == "1" ]]; then
    return 0
  fi
  return 1
}
...
# ----- APT bootstrap (tools + prime wordlists) -----
apt_bootstrap() {
...
menu(){
  while true; do
    tty_clear
    echo "=== Update Git & Wordlists — Interactive ==="
    print_config
    cat <<MENU
Choose an option:
...
# --- Header shown on terminal only (log header appended separately) ---
echo "${C_INFO}Run date: $(date)${C_RESET}"
echo "${C_INFO}Run log will be saved to:${C_RESET} $RUN_LOG"

# If interactive, run menu first (no tee yet)
$SHOW_MENU && menu

# Apply Include Assetnote choice (after menu)
if $INCLUDE_ASSETNOTE; then
  WORDLIST_REPOS["assetnote-wordlists"]="https://github.com/assetnote/wordlists.git"
fi

# Start logging to both terminal & file AFTER the menu.
# Terminal keeps colors; file gets ANSI stripped.
exec > >(tee >(sed -r 's/\x1B\[[0-9;]*[mK]//g' >> "$RUN_LOG")) 2>&1

# Helper to write ONLY to log (no terminal).
log_only() {
...
# === 2) Clone/update 3rd-party wordlists ===
...
# === 3) Auto-decompress archives (.tar.gz/.tgz + single .gz) ===
